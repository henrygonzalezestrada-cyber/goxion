(()=>{
  ...
  window.gxSaveBenefit=async function(){const key=document.getElementById('gx-benefit-key').value,c=(typeof clientesDict!=='undefined'?clientesDict?.[key]:null);if(!c?._id)return;const btn=document.getElementById('gx-benefit-save');btn.disabled=true;btn.textContent='Guardando…';try{await window.GOXION_FINANCIAL_ACTIONS.saveBenefit({clienteId:c._id,concepto:document.getElementById('gx-benefit-concept').value.trim()||'Descuento programado',tipo:document.getElementById('gx-benefit-type').value,valor:Number(document.getElementById('gx-benefit-value').value),periodoInicio:document.getElementById('gx-benefit-period').value,periodosTotal:Number(document.getElementById('gx-benefit-duration').value)});gxCloseBenefitModal();await window.gxReloadAdminClient?.(key);}catch(e){alert('❌ No se pudo programar el descuento.\n\n'+(e?.message||e));}finally{btn.disabled=false;btn.textContent='Programar descuento';}};
  window.gxCancelBenefit=async function(id,key){if(!confirm('¿Cancelar este beneficio? Dejará de aplicarse desde el próximo cálculo.'))return;try{await window.GOXION_FINANCIAL_ACTIONS.cancelBenefit({id});await window.gxReloadAdminClient?.(key);}catch(e){alert('❌ No se pudo cancelar.\n\n'+(e?.message||e));}};
})();
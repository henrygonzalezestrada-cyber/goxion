(()=>{
  ...
  window.gxPromoSave=async function(){
    ...
    try{await window.GOXION_FINANCIAL_ACTIONS.savePromotion(data);gxPromoCloseEditor();await gxPromoLoad();}
  ...
  window.gxPromoToggle=async function(id,activa){try{await window.GOXION_FINANCIAL_ACTIONS.setPromotionState({id,activa});await gxPromoLoad();}catch(e){alert(`No se pudo cambiar el estado.\n\n${e?.message||e}`)}};
